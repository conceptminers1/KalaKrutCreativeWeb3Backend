...
const ArtistProfile: React.FC<ArtistProfileProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<
    'overview' | 'invest' | 'settings' | 'financials' | 'leads' | 'guide'
  >('overview');
  ...
  const isSubscribed =
    localArtist.subscription?.status === 'Active' &&
  ...
  const setDefaultMethod = (type: 'crypto' | 'fiat', id: string) => {
    ...
    if (type === 'crypto') {
      updated.savedPaymentMethods.crypto =
        updated.savedPaymentMethods.crypto.map((m) => ({
          ...m,
          isDefault: m.id === id,
        }));
      updated.savedPaymentMethods.fiat = updated.savedPaymentMethods.fiat.map(
        (m) => ({ ...m, isDefault: false })
      );
    } else {
      updated.savedPaymentMethods.fiat = updated.savedPaymentMethods.fiat.map(
  ...
  const handleAcceptDaoInvite = () => {
    ...
    notify(
      'Welcome to the DAO! You are now an active voting member.',
      'success'
    );
  };
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        if (typeof reader.result === 'string') {
          ...
          notify(
            `${field === 'avatar' ? 'Avatar' : 'Cover Image'} updated successfully!`,
            'success'
          );
        }
      };
  ...
  const handleRemoveSocial = (index: number) => {
    const updatedSocials = localArtist.pressKit.socials.filter(
      (_, i) => i !== index
    );
    setLocalArtist((prev) => ({
      ...prev,
  ...
  const PayoutModal = () => {
    ...
    const [amount, setAmount] = useState(
      localArtist.revenue?.pendingPayout || 0
    );
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
        <div className="bg-kala-900 border border-kala-700 rounded-2xl w-full max-w-md shadow-2xl overflow-hidden animate-in fade-in zoom-in-95">
          ...
          <div className="p-6 space-y-6">
            <div>
              <label className="block text-xs text-kala-500 font-bold uppercase mb-2">
                Withdrawal Amount (USD)
              </label>
              <div className="relative">
                <span className="absolute left-4 top-3 text-white font-bold">
                  $
                </span>
                <input
                  type="number"
                ...
                />
                <button
                  className="absolute right-3 top-2.5 text-xs bg-kala-700 px-2 py-1 rounded text-kala-300 hover:text-white"
                  onClick={() =>
                    setAmount(localArtist.revenue?.pendingPayout || 0)
                  }
                >
                  Max
                </button>
              ...
              <p className="text-xs text-kala-500 mt-2">
                Available Balance: $
                {(localArtist.revenue?.pendingPayout || 0).toLocaleString()}
              </p>
            </div>
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-xs text-kala-500">
                          {m.network} â€¢ {m.address.substring(0, 6)}...
                        </div>
                      </div>
                    </div>
                    {selectedId === m.id && (
            ...
            <button
              ...
              onClick={() => {
                notify(
                  `Withdrawal of $${amount} requested to destination ${selectedId}`,
                  'info'
                );
  ...
  const revenue = localArtist.revenue || {
    totalLifetime: 0,
    pendingPayout: 0,
    lastMonth: 0,
    breakdown: { gigs: 0, merch: 0, royalties: 0, licensing: 0 },
    recentPayouts: [],
  };
            ...
            <div className="flex items-end gap-4">
              ...
              <div className="mb-1">
                ...
                <div className="flex items-center gap-2 text-kala-300 text-sm mt-1">
                  ...
                  {localArtist.genres?.join(', ') || 'No genres specified'}
                </div>
              </div>
            </div>
      ...
      {activeTab === 'overview' ? (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-in fade-in">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-kala-800/50 border border-kala-700 rounded-xl p-6">
              <h3 className="text-lg font-bold text-white mb-4 flex items-center gap-2">
                <Play className="w-5 h-5 text-kala-secondary" /> Top Tracks /
                Content
              </h3>
              <div className="space-y-2">
                {localArtist.pressKit.topTracks.length > 0 ? (
                  localArtist.pressKit.topTracks.map((track, i) => (
                    <div
                      key={i}
                      className="flex items-center justify-between p-3 rounded-lg hover:bg-kala-800 transition-colors group cursor-pointer"
                    >
                      <div className="flex items-center gap-3">
                        <span className="text-kala-500 text-sm font-mono w-4">
                          {i + 1}
                        </span>
                        <div className="font-medium text-white group-hover:text-kala-secondary">
                          {track.title}
                        </div>
                      </div>
                  ...
                  <div className="text-kala-500 text-sm italic">
                    No tracks available.
                  </div>
                )}
              </div>
          ...
          <div className="space-y-6">
            <div className="bg-kala-800/50 border border-kala-700 rounded-xl p-6">
              <h3 className="text-sm font-bold text-kala-500 uppercase mb-4">
                Performance Stats
              </h3>
              <div className="space-y-4">
                <div className="flex justify-between items-center">
                  <span className="text-kala-400 text-sm">
                    Community Rating
                  </span>
                  <span className="text-white font-bold flex items-center gap-1">
                    <Star className="w-4 h-4 text-yellow-400 fill-yellow-400" />{' '}
                    {localArtist.stats.rating}
                  </span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-kala-400 text-sm">
                    Gigs/Deals Completed
                  </span>
                  <span className="text-white font-bold">
                    {localArtist.stats.gigsCompleted}
                  </span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-kala-400 text-sm">Avg. Response</span>
                  <span className="text-white font-bold">
                    {localArtist.stats.responseTime}
                  </span>
                </div>
              </div>
            </div>
        ...
        <div className="space-y-6 animate-in slide-in-from-right-4 fade-in">
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                <h3 className="text-xl font-bold text-white mb-2">
                  Invest in {localArtist.name}'s Future
                </h3>
                <p className="text-kala-300 text-sm max-w-2xl">
                  Purchase equity tokens to support upcoming projects and share
                  in the revenue. All contracts are audited and executed via
                  KalaKrut DAO protocols.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {localArtist.equityOpportunities?.map((opp) => (
              <div
                key={opp.id}
                className="bg-kala-800/50 border border-kala-700 rounded-xl p-6 hover:border-kala-500 transition-colors"
              >
                ...
                <p className="text-sm text-kala-400 mb-4 h-10 line-clamp-2">
                  {opp.description}
                </p>
                ...
                <div className="bg-kala-900/50 rounded-lg p-3 space-y-2 mb-4">
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="text-white font-mono">
                      {opp.totalValuation.toLocaleString()} {opp.currency}
                    </span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-kala-500">Equity Offered</span>
                    <span className="text-white font-mono">
                      {opp.equityAvailablePercentage}%
                    </span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-kala-500">Min. Investment</span>
                    <span className="text-white font-mono">
                      {opp.minInvestment} {opp.currency}
                    </span>
                  </div>
                </div>
        ...
        <div className="space-y-6 animate-in slide-in-from-right-4 fade-in">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-kala-800 border border-kala-700 rounded-xl p-6">
              ...
              <div className="text-3xl font-mono text-white">
                ${revenue.totalLifetime.toLocaleString()}
              </div>
            </div>
            <div className="bg-kala-800 border border-kala-700 rounded-xl p-6">
              <div className="text-kala-400 text-sm font-bold uppercase mb-2">
            ...
            <div className="bg-kala-800 border border-kala-700 rounded-xl p-6">
              ...
              <div className="text-3xl font-mono text-green-400">
                +${revenue.lastMonth.toLocaleString()}
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="bg-kala-800/50 border border-kala-700 rounded-xl p-6">
              ...
              <div className="space-y-4">
                {Object.entries(revenue.breakdown || {}).map(([key, val]) => (
                  <div key={key}>
                    ...
                    <div className="w-full bg-kala-900 rounded-full h-2">
                      <div
                        ...
                        style={{
                          width: `${((val as number) / (revenue.totalLifetime || 1)) * 100}%`,
                        }}
                      ></div>
                    </div>
                  </div>
            ...
            <div className="bg-kala-800/50 border border-kala-700 rounded-xl p-6">
              ...
              <div className="space-y-3">
                {revenue.recentPayouts.map((payout, i) => (
                  <div
                    key={i}
                    className="flex justify-between items-center p-3 bg-kala-900/50 rounded-lg"
                  >
                    <div>
                ...
                {(!revenue.recentPayouts ||
                  revenue.recentPayouts.length === 0) && (
                  <div className="text-center text-kala-500 text-sm py-4">
        ...
        <div className="space-y-6 animate-in slide-in-from-right-4 fade-in">
          ...
          <div className="bg-kala-900/50 border border-kala-800 rounded-xl p-6">
            <h4 className="text-sm font-bold text-white mb-3 flex items-center gap-2">
              {' '}
              <Bot className="w-4 h-4 text-indigo-400" /> Gemini LeadGenius
              Log{' '}
            </h4>
            <div className="space-y-3">
              <input
            ...
            <p className="text-[10px] text-kala-500 mt-2">
              * Manual recording is included in your current plan. To automate
              this, activate Auto-Sync in the Services Hub.
            </p>
          </div>
          ...
          <div className="bg-kala-900/50 border border-kala-800 rounded-xl p-6">
            <h4 className="text-sm font-bold text-white mb-3 flex items-center gap-2">
              <Search className="w-4 h-4 text-teal-400" /> MusicBrainz Artist
              Search
            </h4>
          ...
          <div className="space-y-4">
            {localArtist.leadQueries?.map((lead) => (
              <div
                key={lead.id}
                className="bg-kala-800 border border-kala-700 rounded-xl p-4 hover:border-kala-500 transition-colors"
              >
                ...
                <div className="mb-3">
                  <div className="text-xs text-kala-400 font-bold uppercase mb-1">
                    Query
                  </div>
                ...
                <div className="bg-kala-900/50 p-3 rounded-lg border border-kala-700/50">
                  ...
                  <div className="text-kala-300 text-sm whitespace-pre-wrap">
                    {lead.responseSummary}
                  </div>
                </div>
              </div>
            ))}
            {(!localArtist.leadQueries ||
              localArtist.leadQueries.length === 0) && (
              <div className="text-center py-8 text-kala-500 text-sm">
                No lead queries recorded yet.
              </div>
            )}
          </div>
        </div>
        ...
        <div className="space-y-6 animate-in slide-in-from-right-4 fade-in">
          ...
          <div className="bg-kala-800/50 border border-kala-700 rounded-xl p-6">
            <h3 className="text-lg font-bold text-white mb-4 flex items-center gap-2">
              <Image className="w-5 h-5 text-kala-secondary" /> Profile
              Appearance
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <div className="flex gap-4 items-center">
                  <img
                    src={localArtist.avatar}
                    alt="Preview"
                    className="w-16 h-16 rounded-lg bg-kala-900 object-cover border border-kala-700"
                  />
              ...
              <div>
                ...
                <div className="flex gap-4 items-center">
                  <img
                    src={localArtist.coverImage}
                    alt="Preview"
                    className="w-24 h-16 rounded-lg bg-kala-900 object-cover border border-kala-700"
                  />
                  <label className="cursor-pointer bg-kala-800 hover:bg-kala-700 text-white px-4 py-2 rounded-lg border border-kala-600 text-xs font-bold flex items-center gap-2 transition-colors">
                    <Upload className="w-4 h-4" /> Upload New
                    <input
          ...
          <div className="bg-kala-800/50 border border-kala-700 rounded-xl p-6">
            ...
            <div className="space-y-3">
              {localArtist.pressKit.socials.map((social, index) => (
                <div
                  key={index}
                  className="flex flex-col md:flex-row gap-3 items-end md:items-center bg-kala-900/30 p-3 rounded-lg border border-kala-700/50"
                >
                  <div className="flex-1 w-full">
                    <label className="text-[10px] text-kala-500 uppercase font-bold">
                      Platform
                    </label>
                    <input
                      type="text"
          ...
          <div
            className={`border rounded-xl p-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 ${isBlocked ? 'bg-red-900/20 border-red-500/50' : 'bg-gradient-to-br from-kala-800 to-kala-900 border-kala-700'}`}
          >
            <div>
              <div className="flex items-center gap-3 mb-2">
                ...
                <span
                  className={`px-2 py-0.5 text-xs font-bold rounded border ${isBlocked ? 'bg-red-500/20 text-red-400 border-red-500/30' : 'bg-green-500/20 text-green-400 border-green-500/30'}`}
                >
              ...
              <div className="space-y-1 text-sm text-kala-400">
                <p className="flex items-center gap-2">
                  ...
                  {isBlocked
                    ...
                    : 'No recent violations found on your account.'}
                </p>
              </div>
            </div>
          </div>
          ...
          <div
            className={`border rounded-xl p-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 ${localArtist.role === UserRole.DAO_MEMBER ? 'bg-purple-900/20 border-purple-500/30' : 'bg-kala-800/50 border-kala-700'}`}
          >
            <div>
              <div className="flex items-center gap-3 mb-2">
                <div
                  className={`p-2 rounded-lg ${localArtist.role === UserRole.DAO_MEMBER ? 'bg-purple-500 text-white' : 'bg-kala-700 text-kala-400'}`}
                >
          ...
          <div className="bg-gradient-to-br from-kala-800 to-kala-900 border border-kala-700 rounded-xl p-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            <div>
              <div className="flex items-center gap-3 mb-2">
                <h3 className="text-2xl font-bold text-white">
                  {localArtist.subscription?.planName} Plan
                </h3>
              ...
              <div className="space-y-1 text-sm text-kala-400">
                <p className="flex items-center gap-2">
                  <Calendar className="w-4 h-4 text-kala-500" /> Renews on:{' '}
                  <span className="text-white">
                    {localArtist.subscription?.expiryDate}
                  </span>
                </p>
                <p className="flex items-center gap-2">
                  <ShieldCheck className="w-4 h-4 text-kala-500" /> Support
                  Tier:{' '}
                  <span className="text-kala-secondary font-bold">
                    {localArtist.subscription?.supportTier}
                  </span>
                </p>
              </div>
            </div>
            ...
            <div className="bg-kala-800/50 border border-kala-700 rounded-xl p-6">
              ...
              <div className="space-y-3">
                {localArtist.savedPaymentMethods?.crypto.map((wallet) => (
                  <div
                    key={wallet.id}
                    className="p-3 bg-kala-900/50 border border-kala-700 rounded-lg flex justify-between items-center group"
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="text-sm font-bold text-white flex items-center gap-2">
                          {wallet.label}
                          {wallet.isDefault && (
                            <span className="text-[10px] bg-purple-500/20 text-purple-400 px-1.5 rounded border border-purple-500/30">
                              DEFAULT
                            </span>
                          )}
                        </div>
                        <div className="text-xs text-kala-400 flex items-center gap-1">
                          <span className="text-kala-500">
                            {wallet.network}
                          </span>{' '}
                          â€¢ {wallet.address}
                        </div>
                    ...
                    <div className="flex items-center gap-2">
                      {!wallet.isDefault && (
                        <button
                          onClick={() => setDefaultMethod('crypto', wallet.id)}
                          className="text-[10px] text-kala-500 hover:text-white px-2 py-1 bg-kala-800 rounded"
                        >
                          Set Default
                        </button>
                      )}
                ...
                {localArtist.savedPaymentMethods?.fiat.map((method) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      {!method.isDefault && (
                        <button
                          onClick={() => setDefaultMethod('fiat', method.id)}
                          className="text-[10px] text-kala-500 hover:text-white px-2 py-1 bg-kala-800 rounded"
                        >
                          Set Default
                        </button>
                      ...
                      <button
                        className="text-kala-500 hover:text-red-400"
                        title="Remove Method"
                      >
                        <Trash2 className="w-4 h-4" />
                      </button>
            ...
            <div className="flex flex-col md:flex-row justify-between items-center gap-6">
              <div>
                ...
                <p className="text-xs text-kala-400 max-w-lg mt-1">
                  Permanently delete your profile, assets, and history. This
                  action cannot be undone.
                  <span className="text-red-300 ml-1">
                    Note: As per community guidelines, you cannot self-delete
                    active accounts to prevent fraud. Please contact an admin.
                  </span>
                </p>
              </div>
              <button
