...
const ServicesHub: React.FC<ServicesHubProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'platform' | 'members'>(
    'platform'
  );
    ...
    <div className="space-y-6 relative">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-kala-400 text-sm">
            Access professional tools and community expertise.
          </p>
        </div>
          ...
          <div className="bg-gradient-to-br from-indigo-900/80 to-kala-900 border border-indigo-500/30 rounded-2xl p-8 relative overflow-hidden group">
            ...
            <div className="relative z-10">
              <div className="flex items-center gap-3 mb-4">
                ...
                <span className="px-2 py-0.5 bg-yellow-500/20 text-yellow-400 text-xs font-bold border border-yellow-500/30 rounded">
                  PREMIUM
                </span>
              </div>
              <p className="text-indigo-200 max-w-2xl mb-6 text-lg">
                Automate your collaboration outreach. Generate high-quality
                leads for venues, sponsors, and artists using our AI-powered
                engine. Perfect for scaling your creative business.
              </p>
              ...
              {userRole === UserRole.ADMIN ? (
                ...
                <p className="text-xs text-indigo-300/60 mt-4">
                  * Users can manually record queries in their profile for free.
                  Auto-Sync is a paid add-on.
                </p>
              )}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-kala-800/50 border border-kala-700 p-6 rounded-xl hover:border-kala-500 transition-colors">
              ...
              <p className="text-kala-400 text-sm mb-4">
                Official platform verification for your custom DAO contracts and
            ...
            <div className="bg-kala-800/50 border border-kala-700 p-6 rounded-xl hover:border-kala-500 transition-colors">
              ...
              <p className="text-kala-400 text-sm mb-4">
                Dedicated support for large-scale festival bookings and
        ...
        <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-500">
          ...
          <div className="bg-kala-800/30 border border-kala-700 rounded-xl p-4 mb-4">
            <p className="text-sm text-kala-300">
              Community providers are vetted members offering specialized
              services.
              <span className="text-kala-500 ml-1">
                (Transactions secured by KalaKrut Escrow)
              </span>
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 gap-4">
            {MEMBER_SERVICES.map((svc) => (
              <div
                key={svc.id}
                className="bg-kala-800/50 border border-kala-700 p-6 rounded-xl flex flex-col md:flex-row items-start md:items-center justify-between hover:border-kala-500 transition-colors group"
              >
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-kala-400 text-sm flex items-center gap-2 mt-1">
                      ...
                      <span className="px-2 py-0.5 rounded-full bg-kala-700/50 text-xs border border-kala-600">
                        {svc.category}
                      </span>
                    </p>
                  </div>
                ...
                <div className="flex items-center gap-8 mt-4 md:mt-0 w-full md:w-auto justify-between md:justify-end">
                  ...
                  <div className="text-right min-w-[100px]">
                    <div className="text-lg font-bold text-kala-secondary font-mono">
                      ${svc.rate}
                    </div>
      ...
      {selectedService && (
        <PaymentGateway
          ...
          onSuccess={(method) => {
            notify(
              `Service Booked via ${method === 'crypto' ? 'Crypto' : 'Fiat'}!`,
              'success'
            );
      ...
      {showAutoSyncPayment && (
        <PaymentGateway
          ...
          onSuccess={(method) => {
            notify(
              'Auto-Sync Activated! Queries will now appear in your profile.',
              'success'
            );
