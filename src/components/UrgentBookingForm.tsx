...
const UrgentBookingForm: React.FC<UrgentBookingFormProps> = ({ artistName, onClose, onSubmit }) => {
  ...
  const advanceAmount = totalRate ? (parseFloat(totalRate) * 0.5).toFixed(2) : '0.00';
  const finalPayout = totalRate ? (parseFloat(totalRate) * 0.5).toFixed(2) : '0.00';
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-70 z-50 flex items-center justify-center p-4 animate-fade-in">
      <div className="bg-kala-800 border border-kala-700 rounded-2xl shadow-2xl w-full max-w-lg relative overflow-hidden">
        ...
        <button onClick={onClose} className="absolute top-4 right-4 text-kala-500 hover:text-white transition-colors">
          <X size={24} />
        </button>
        ...
        <div className="p-8">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h2 className="text-2xl font-bold text-white">Urgent Booking Request</h2>
          </div>
          <p className="text-kala-400 mb-6">Requesting <span className="font-bold text-white">{artistName}</span> for an event in the next 0-6 days.</p>

          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="text-sm font-bold text-kala-300 mb-2 block">Event Name</label>
              <input
                type="text"
                value={eventName}
            ...
            <div>
              <label htmlFor="days" className="text-sm font-bold text-kala-300 mb-2 block">Days Until Event</label>
              <select
                id="days"
            ...
            <div>
                <label className="text-sm font-bold text-kala-300 mb-2 block">Total Rate (USD)</label>
                <div className="relative">
                    <DollarSign className="absolute left-3 top-1/2 -translate-y-1/2 text-kala-500" size={18} />
                    <input
                        type="number"
                        value={totalRate}
            ...
            <div className="grid grid-cols-2 gap-4 text-center bg-kala-900/50 p-3 rounded-lg border border-kala-700">
                <div>
                    <div className="text-lg font-bold text-green-400">${advanceAmount}</div>
                    <div className="text-xs text-kala-400">50% Advance Booking</div>
                </div>
                <div>
                    <div className="text-lg font-bold text-white">${finalPayout}</div>
                    <div className="text-xs text-kala-400">50% Final Payout</div>
                </div>
            </div>

            <div>
                <label className="text-sm font-bold text-kala-300 mb-2 block">Message (Optional)</label>
                <textarea
                    value={message}
